"""Green Agent executor - "How to invoke it" (protocol adapter)

Thin wrapper that converts:
- A2A protocol params → Agent.run() → A2A protocol response
- Task lifecycle management only (working, completed, failed states)
- No domain logic (evaluation logic belongs in agent.py)

Pattern: executor.py = Protocol adapter, separated from domain (agent.py)
