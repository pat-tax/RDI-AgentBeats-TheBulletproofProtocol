"""Tests for Dockerfile.green build and runtime behavior.

These tests verify that the green agent Docker container builds successfully
and runs the green agent server on startup.
"""
...
def test_dockerfile_green_builds_successfully() -> None:
    """Test that Dockerfile.green builds successfully on linux/amd64 platform."""
...
def test_dockerfile_green_exposes_port_8000() -> None:
    ...
    result = subprocess.run(
        ["docker", "inspect", "test-green:latest"],
        capture_output=True,
        text=True,
        check=True,
    )
...
def test_dockerfile_green_runs_server() -> None:
    """Test that docker run successfully starts the green agent server."""
...
def test_dockerfile_green_includes_python_313() -> None:
...
def test_dockerfile_green_includes_a2a_sdk() -> None:
